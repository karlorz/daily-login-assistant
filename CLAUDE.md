...
```bash
feat: add new feature          # Minor release (1.4.0 → 1.5.0)
fix: resolve bug              # Patch release (1.4.0 → 1.4.1)
docs: update documentation    # NO RELEASE (skipped by semantic-release)
style: format code            # NO RELEASE (skipped by semantic-release)
chore(docs): update guide     # NO RELEASE (skipped by semantic-release)
perf: improve performance     # Patch release (1.4.0 → 1.4.1)
feat(api)!: breaking change   # Major release (1.4.0 → 2.0.0)
```

**Semantic-Release Configuration** (`.releaserc.json`):
- Documentation-only commits (docs, style, chore:docs) do NOT trigger releases
- This prevents version bumps for README updates, typo fixes, etc.
- Feature/fix commits still trigger appropriate releases
...
- [x] **Production Deployment** (Oct 2025):
  - v1.4.0 deployed to dailycheckin.lan:8001
  - Auto-configured anyrouter endpoint based on hostname
  - PWA port configuration fixed (PWA_PORT environment variable)
  - Bun runtime integrated for production
  - Systemd service with auto-hostname detection
- [x] **Documentation Consolidation** (Oct 2025):
  - Comprehensive README.md created
  - 13 root markdown files archived to docs/archive/
  - Semantic-release configured to skip docs-only changes
...
### Current Release
**Version**: v1.4.0 (October 12, 2025)
- Auto-configure anyrouter endpoint based on deployment hostname
- Fix PWA_PORT environment variable for web API port configuration
- Comprehensive deployment improvements documentation
- Production-ready for headless LXC deployment with profile-based authentication
...
### Main Documentation
- **[README.md](README.md)** - **PRIMARY**: Comprehensive project documentation with quick start guide
- **[CLAUDE.md](CLAUDE.md)** - **THIS FILE**: Complete project instructions for AI assistants
- **[CHANGELOG.md](CHANGELOG.md)** - Automated release history (generated by semantic-release)

### Archived Documentation (docs/archive/)
All detailed guides have been consolidated into README.md. Original files are preserved in `docs/archive/`:
- [Docker Deployment Guide](docs/archive/DOCKER_DEPLOYMENT.md) - Complete guide for headless Docker deployment
- [Deployment Checklist](docs/archive/DEPLOYMENT_CHECKLIST.md) - Step-by-step deployment validation
- [Deployment Improvements](docs/archive/DEPLOYMENT_IMPROVEMENTS.md) - Oct 2025 deployment session summary
- [Multi-Profile System](docs/archive/PROFILE_SYSTEM.md) - Unified site + user management
- [PWA Quickstart](docs/archive/PWA_QUICKSTART.md) - PWA remote server quick guide
- [PWA Deployment](docs/archive/PWA_DEPLOYMENT.md) - Detailed PWA deployment
- [Remote Deployment Guide](docs/archive/REMOTE_DEPLOYMENT_GUIDE.md) - Remote Docker deployment
- [Local Dev Setup](docs/archive/LOCAL_DEV_SETUP.md) - Development environment setup
- [Testing Guide](docs/archive/TESTING_GUIDE.md) - Testing procedures
- [Implementation TODO](docs/archive/IMPLEMENTATION_TODO.md) - Implementation status tracker
- [CDP Remote Debugging Architecture](docs/archive/CDP_REMOTE_DEBUGGING_ARCHITECTURE.md) - Chrome DevTools Protocol details
